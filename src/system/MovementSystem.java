...
public class MovementSystem {
    ...
    public void updatePlayer(Entity player) {
        ...
        collisionSystem.collidesT(player); // check tile collision
        collisionSystem.collidesWithObject(player);

        if (player.collisionOn) {
            player.x -= player.velocityX; // undo move
            player.velocityX = 0;
        }
        ...
        collisionSystem.collidesWithObject(player);
        if (player.collisionOn) {
            player.y -= player.velocityY; // undo move
    ...
    public void updateOpponent(Entity opponent, char[] directions, java.util.Random random) {
        ...
        collisionSystem.collidesT(opponent);
        if (opponent.collisionOn) {

            opponent.x -= opponent.velocityX;
            opponent.y -= opponent.velocityY;

            char newDirection = directions[random.nextInt(2)];
            updateOpponentDirection(opponent, newDirection);
        }

        if (opponent.x <= 0 || opponent.x + opponent.width >= boardWidth) {
    ...
    public void updateOpponentDirection(Entity opponent, char direction) {
...
//        }
    ...
    public void updatePlayerDirection(Entity player, char direction, HashSet<Entity> walls) {
        ...
        // Revert direction if collision occurs with new direction
//        for (Entity wall : walls) {
//            if (collisionSystem.collidesT(player, wall)) {
//                player.y -= player.velocityY;
